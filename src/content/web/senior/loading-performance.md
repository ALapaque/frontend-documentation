...
- **`type="module"`** : implique `defer` automatiquement (téléchargement parallèle, exécution après parsing) ; supporte `import`. En 2026 c'est le défaut moderne — les baselines de navigateurs supportent les modules ES nativement, plus besoin de transpiler vers des bundles legacy pour la majorité du trafic.
...
L'attribut **`fetchpriority`** ajuste la priorité que le navigateur assigne par défaut : `high` pour pousser l'image LCP devant les images décoratives, `low` pour rétrograder une carrousel hors écran. Il complète les hints — `preload` dit *quoi* charger tôt, `fetchpriority` dit *à quel point* c'est urgent dans la file.
...
- title: "type=module"
  desc: "Différé par défaut, supporte import ; défaut moderne en 2026."
